 ...
 <div class="modal-dialog">
  <div class="modal-content">
   ...
   <tr><td align="center" width="300">Descripción de Bienes/Servicios</td></tr>
    ...
    $res=mysqli_query($conexion,$sql);
    ?>
    <select name="rubro" size=1 id="rr" onchange="cargarsub()">
    <option value="0">Seleccione</option>
     <?php while ($fila=mysqli_fetch_array($res)){ 
    ?>
  ...
  <?php }?>
    ...
    ?>
     <script type="text/javascript">
     function cargarsub(){
     //tomar el id de rubro,recorrer y mostrar solo los que coincidan con el
     //id de subrubro
       var element = document.getElementById("rr");
       //document.write(element.value); 
       tomarelemento(element);
//       mostrarsubr(element);
       }
     </script>
     <tr>
    ...
    <select name="subrubro" size=1 id="srr">
    <option value="0">Seleccione</option>
     ...
     <?php while ($fila=mysqli_fetch_array($res2)){ 
         ...
         $qq=$fila['subrubro'];?>
         <script type="text/javascript">
         function tomarelemento(element){
          var pp= "<?php echo $pp; ?>" ;
          var qq= "<?php echo $qq; ?>" ;
          if(pp==element.value){
           //window.alert(qq); 
           //no muestra cuando coinciden lo
           //guardar en array definido e ir corriendo una posicion
           mostrarsubr(qq);
          }
  ...
  <?php }?>
  <script type="text/javascript">
  function mostrarsubr(qq){
      var sr=document.getElementById('srr');
      for (var i = 0; i < sr.length; i++) 
     {
      var opt = sr[i];
      if(opt.value==qq){
        opt.setAttribute('enable','enable');
    }else{opt.setAttribute('disable','disable');}
    }
  }
     </script>
