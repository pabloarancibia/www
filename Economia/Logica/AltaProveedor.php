  ...
  if($existe==TRUE)
	{
        ...
        if($ec==6){$estadocivil="OTRO";} $dni=$_POST['dni'];
        ...
        $rs=mysqli_query($conexion,$query)or die(include("../views/frmAltaProveedor.php"));		
		    $respuesta = "CARGA DE DATOS CORRECTA";
         //aca actualiza el estado P de usuarios 		
	     actualizarEstado($nroprov,$razonsocial);	
        include("../views/frmAltaProveedor.php");
		 //$flag=false;
...
/////////////////////////////////////////////
function actualizarEstado($nroprov,$razonsocial){
   $nn=$nroprov; $es="A";$rz=$razonsocial;
 $queryAc="UPDATE usuarios SET razonsocial='".$rz."', estado='".$es."' where nroprov='".$nn."' ;";
