...
if (empty($_POST['optDif'])){
  $optDif='00';
}else{$optDif=$_POST['optDif'];}//6a5
//si optDif si
...
$queryGuardarRS = "INSERT INTO razonsocial (idRazonSocial,razonsocial,cuit,domicilio,codActMuni,codActAtp,email,cel)
VALUES ('".$idrz."','".$razonsocial."','".$cuit."','".$domicilio."','".$codActMuni."','".$codActAtp."','".$email."','".$cel."')";

//compruebo si existe la rs -- num cuit
$idRz_query=mysqli_query($conexion,"SELECT idRazonSocial FROM razonsocial WHERE cuit = '".$cuit."'");//traigo el idRazonSocial para guardarlo en tabla sucursales
...
///////////////////////////////////////////////
function buscarRZ(){
	$queryID = "SELECT idRazonSocial FROM razonsocial ORDER BY idRazonSocial DESC LIMIT 1";
	$conexion=Conectarse();
	$rs =mysqli_query($conexion,$queryID);
	$tot=mysqli_num_rows($rs);
    if ($tot!=0) {
    	$row=@mysqli_fetch_array($rs);
        $id = $row['idRazonSocial']+ 1;
	   }else{$id=1;}
	return $id;
